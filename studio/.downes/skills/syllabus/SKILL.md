...
- course_title (required) · learning_objectives (read `objectives.md` if
  present) · duration_weeks (required) · modality (default: in-person)
- prerequisites (optional) · modules_count (default: 4)

## Method

You are a curriculum designer. Draft a concise module-by-module syllabus
outline using `### Module X: Title` headings in order. For each module
include:
...
Keep the tone practical and avoid extra commentary outside the requested
structure.

## Output contract

Write `syllabus.md` in the current course folder. The file must contain
only, in this order:
