...
def get_financial_metrics_snapshot(ticker: str) -> dict:
    ...
    """
    return _provider.get_financial_metrics_snapshot(ticker)
...
class FinancialMetricsInput(BaseModel):
    ...
    limit: int = Field(
        default=4, description="The number of past financial statements to retrieve."
    )
    ...
    report_period_gt: Optional[str] = Field(
        default=None,
        description="Filter for financial metrics with report periods after this date (YYYY-MM-DD).",
    )
    report_period_gte: Optional[str] = Field(
        default=None,
        description="Filter for financial metrics with report periods on or after this date (YYYY-MM-DD).",
    )
    report_period_lt: Optional[str] = Field(
        default=None,
        description="Filter for financial metrics with report periods before this date (YYYY-MM-DD).",
    )
    report_period_lte: Optional[str] = Field(
        default=None,
        description="Filter for financial metrics with report periods on or before this date (YYYY-MM-DD).",
    )
...
def get_financial_metrics(
    ...
    """
    return _provider.get_financial_metrics(
        ticker,
        period,
        limit,
