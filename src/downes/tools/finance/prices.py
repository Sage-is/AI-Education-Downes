...
def get_price_snapshot(ticker: str) -> dict:
    ...
    """
    return _provider.get_price_snapshot(ticker)
...
class PricesInput(BaseModel):
    ...
    interval: Literal["minute", "day", "week", "month", "year"] = Field(
        default="day",
        description="The time interval for price data. Defaults to 'day'.",
    )
    interval_multiplier: int = Field(
        default=1, description="Multiplier for the interval. Defaults to 1."
    )
