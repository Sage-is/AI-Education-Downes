...
class VerifyAndSummarizeInput(BaseModel):
    ...
    topic: str = Field(description="The curriculum topic to assess relevance against.")
...
You are a curriculum resource evaluator. Given fetched web page content and a \
curriculum topic, produce a structured assessment.
...
CURRICULUM_USE: One sentence suggesting how an educator could use this resource.
...
def verify_and_summarize(
    url: str, topic: str, audience: Optional[str] = None
) -> str:
    """Fetch a URL, verify it is accessible, and produce a structured summary
    with relevance assessment for curriculum development."""
    ...
    curriculum_use = fields.get("CURRICULUM_USE", "")
